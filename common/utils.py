...
def get_limit_date():
    ...
    now = datetime.now()
    if now.day <= 15:
        limit = now - relativedelta(months=2)  # 수집 기간 설정
    else:
        limit = now - relativedelta(months=1)  # 수집 기간 설정
...
def is_excluded(title):
    ...
    for ex_kw in KEYWORD_CATEGORIES.get(0, []):
        if ex_kw and ex_kw.strip().lower() in title_low:
            return True
    return False
...
def process_article(article):
    ...
    if is_excluded(article.get("title", "")):
        return None
