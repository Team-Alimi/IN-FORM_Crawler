...
class TypeDSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i, row in enumerate(rows):
            ...
            # 날짜 추출
            dt_txt = "".join(
                row.css("td:nth-child(2)::text, td:nth-child(2) *::text").getall()
            ).strip()
            dt_obj = parse_date_raw(dt_txt)

            # 고정 공지 판정
            has_icon = (
                row.css('img[src*="notice"], span.mb-notice, .mb-notice').get()
                is not None
            )
            title_link = row.css("td.text-left a")
            title = title_link.css("::attr(title)").get() or "".join(
                title_link.css("::text").getall()
            )
            title = title.strip()
            ...
            if lnk:
                yield response.follow(
                    ...
                    cb_kwargs={"title": title, "num": num, "dt_obj": dt_obj},
                )
    ...
    def parse_detail(self, response, title, num, dt_obj):
        ...
        view = response.css(
            'tr[id*="tr_content"] td, div.board-view-cnt, .view_content, #board_view_content, .mb-content'
        )
        if not view:
        ...
        art_num = num
        c_at = u_at = (
            format_date_str(dt_obj) if dt_obj else format_date_str(datetime.now())
        )
        ...
        article["unique_id"] = source_identity["external_key"]
        article["title"] = title
        article["content"] = raw_html
        article["original_url"] = source_identity["source_url"]
