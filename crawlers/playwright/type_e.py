...
class TypeECrawler(BaseCrawler):
        ...
        while True:
            ...
            for i in range(rows_count):
                try:
                    ...
                    dt_txt = (
                        await row.locator("._artclTdRdate").first.inner_text()
                    ).strip()
                    ...
                    if href:
                        abs_url = await self.page.evaluate(
                            "(src) => new URL(src, document.baseURI).href", href
                        )
                ...
                try:
                    if await self.safe_goto(item["url"]):
                        ...
                        await self.parse_detail(
                            item["title"], item["dt_obj"], item["art_num"]
                        )
                        await asyncio.sleep(0.5)
                except Exception as e:
                    self.log(
                        f"상세 페이지 접근 오류 ({item['title'][:10]}): {e}", "WARN"
                    )
    ...
    async def parse_detail(self, title, dt_obj, art_num):
        ...
        try:
            ...
            source_identity = self.build_source_identity(art_num, self.page.url)
